...
export function initialize(workspace, agentDirectory = join(workspace, '.pi-experiment-ops/agent')) {
  ...
  const initial = {
    ...
    [join(workspace, '.pi/mcp.json')]: { mcpServers: {} },
  };
...
export function configureEnvironment(workspace, { dataDirectory = join(workspace, '.pi-experiment-ops'), agentDirectory = join(dataDirectory, 'agent') } = {}) {
